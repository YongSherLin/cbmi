...
flowScheduler.add(SaveDataRoutineBegin());
flowScheduler.add(SaveDataRoutineEachFrame());
flowScheduler.add(SaveDataRoutineEnd());
...
async function experimentInit() {
  ...
  neg_option = new visual.TextStim({
    win: psychoJS.window,
    name: 'neg_option',
    text: '',
    font: 'Arial',
  ...
  pos_option = new visual.TextStim({
    win: psychoJS.window,
    name: 'pos_option',
    text: '',
    font: 'Arial',
...
function TrialRoutineBegin(snapshot) {
  return async function () {
    ...
    _key_resp_2_allKeys = [];
    feedback_text.setColor(new util.Color(feedbackColor));
    ...
    TrialComponents.push(neg_option);
    TrialComponents.push(pos_option);
...
function TrialRoutineEachFrame() {
  return async function () {
    ...
    // *neg_option* updates
    if (t >= 0.0 && neg_option.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      neg_option.tStart = t;  // (not accounting for frame time here)
    ...
    // *pos_option* updates
    if (t >= 0.0 && pos_option.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      pos_option.tStart = t;  // (not accounting for frame time here)
...
var SaveDataMaxDurationReached;
var SaveDataMaxDuration;
var SaveDataComponents;
function SaveDataRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //--- Prepare to start Routine 'SaveData' ---
    t = 0;
    SaveDataClock.reset(); // clock
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    SaveDataMaxDurationReached = false;
    // update component parameters for each repeat
    psychoJS.experiment.addData('SaveData.started', globalClock.getTime());
    SaveDataMaxDuration = null
    // keep track of which components have finished
    SaveDataComponents = [];
    
    for (const thisComponent of SaveDataComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function SaveDataRoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'SaveData' ---
    // get current time
    t = SaveDataClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
    // check for quit (typically the Esc key)
    if (psychoJS.experiment.experimentEnded || psychoJS.eventManager.getKeys({keyList:['escape']}).length > 0) {
...
function SaveDataRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('SaveData.stopped', globalClock.getTime());
...
function EndScreenRoutineBegin(snapshot) {
  return async function () {
    ...
    // keep track of which components have finished
    EndScreenComponents = [];
    EndScreenComponents.push(text_2);
